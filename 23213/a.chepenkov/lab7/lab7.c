...
void print_line(const char* map, off_t* offsets, int line_num, int total_lines) {
    if (line_num < 1 || line_num > total_lines) {
        fprintf(stderr, "Wrong string number. Enter number from 1 to %d.\n", total_lines);
        return;
    }

    off_t start = offsets[line_num - 1];
    off_t end = (line_num == total_lines) ? strlen(map) : offsets[line_num];
...
int main(int argc, char* argv[]) {
    ...
    off_t* offsets = malloc((file_size + 1) * sizeof(off_t));
    if (!offsets) {
        perror("malloc");
