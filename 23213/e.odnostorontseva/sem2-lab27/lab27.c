    ...
    while (server_running) {
        ...
        for (int i = 0; i < num_fds; i++) {
            if (fds[i].revents & POLLIN) {
                if (i == 0) {
                    handle_new_connection(server_socket, remote_host, remote_port, 
                                        fds, connections, &num_fds);
                } else {

                    Connection *con = &connections[(i - 1) / 2];
                    if (fds[i].fd == con->client_fd) {
                        handle_client_data(con, fds, &num_fds, i);
                    } else {
                        handle_server_data(con, fds, &num_fds, i);
                    }
                }
            }
        }
...
void handle_client_data(Connection *con, struct pollfd *fds, int *num_fds, int index) {
    handle_data(con->client_fd, con->server_fd, con, fds, num_fds, index);
}

void handle_server_data(Connection *con, struct pollfd *fds, int *num_fds, int index) {
    handle_data(con->server_fd, con->client_fd, con, fds, num_fds, index);
}
