...
void create_connection() {
    ...
    connections[num_connections].events = POLLIN | POLLOUT;
...
void close_connection(struct socket_pair * pair, int closed_fd, int closed_fd_connection_index) {
    int other_fd = (pair->client == closed_fd) ? pair->server : pair->client;
    ...
    struct pollfd pfd = { .fd = other_fd, .events = POLLOUT };
    int ret = poll(&pfd, 1, 0);

    if (ret > 0 && (pfd.revents & POLLOUT)) {
        write(other_fd, buffer, buffer_size);
    }

    free(pair);

    int connections_index = (pair->client == closed_fd) ? closed_fd_connection_index : closed_fd_connection_index - 1;
...
void run_proxy() {
    ...
    while (1) {
        ...
        for(int i = 1; i < num_connections+1; i++) {
