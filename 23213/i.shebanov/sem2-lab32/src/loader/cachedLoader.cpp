...
void CachedLoader::fetch()
{
    ...
    HttpCheckStatus http_version_check = AGAIN;
    ...
    while (true)
    {
        ...
        if (http_version_check == AGAIN && (http_version_check = checkHttpVersion(response)) == FAILED)
        {
            loader_producer_.removeHashedLoader(request_);
            const char *error_message = 
                ...
                "Connection: close\r\n"
                "\r\n"
                "Received unsupported HTTP version from upstream\r\n"
                "\r\n";
            size_t error_message_length = strlen(error_message);
            next_chunk = std::make_shared<Chunk>(error_message, error_message_length, true, false);
            cache->push_front(next_chunk);
            pthread_mutex_lock(&mutex_block_client);
            pthread_cond_broadcast(&cond_block_client);
            pthread_mutex_unlock(&mutex_block_client);
            break;
        ...
        {
            std::cout << "Switched to non cachable loaders" << std::endl;
...
HttpCheckStatus checkHttpVersion(std::string content)
{
    //Return PASSED;
    ...
    if (content.substr(0, first_line_end).find("HTTP/1.0") == std::string::npos)
    {
        return FAILED;
    }
...
{
    std::cout << "Length :" << content.length() << "MAX: " << MAX_CACHABLE_SIZE << std::endl;
