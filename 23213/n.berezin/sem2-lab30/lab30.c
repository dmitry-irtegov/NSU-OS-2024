...
void *terminal_thread(void *arg) {
    ...
    while (1) {
        ...
        const size_t ented_on_i = response_buffer->head;

        if (started_from_i < ented_on_i) {
            write(STDOUT_FILENO, &response_buffer->data[started_from_i], ented_on_i - started_from_i);
        } else {
            write(STDOUT_FILENO, &response_buffer->data[started_from_i], READ_BUF_SIZE - started_from_i);
            write(STDOUT_FILENO, &response_buffer->data[0], ented_on_i);
        }
