...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
        ...
        if (FD_ISSET(sockfd, &read_fds))
        {
            ...
            char *buffer_ptr = buffer + read_shift;
            char *line_buffer_ptr = line_buffer;
            char *end = buffer + read_shift + bytes_to_print;
            while (buffer_ptr < end && printing && rest_bytes < BUFSIZ)
            {   
                if (*buffer_ptr == '\n')
                {
                    *line_buffer_ptr = '\0';
