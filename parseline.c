/**
 * @file parseline.c
 * @brief Command line parsing implementation for shell
 *
 * This file implements the command line parsing functionality including:
...
/**
 * @brief Skip whitespace characters in input string
 * 
 * Advances the pointer past any whitespace characters
 * until a non-whitespace character is found or end of string.
 *
 * @param s Pointer to current position in string
 ...
 * 
 * Parses input string and fills job structure with:
 * - Command arguments
 * - I/O redirection information
 * - Command flags
 * - Process information
 *
 * @param line Input string to parse
 * @return Pointer to job structure on success, NULL on error
 */
job* parseline(char *line){
    ...
    j->next = NULL;
    j->prev = NULL;
    j->appfile = j->outfile = j->infile = j->errfile = (char*) NULL;  
    j->idx = 0;
    nargs = ncmds = rval = 0;
    int curr_idx = 0;
    int pipe_flag = 0;
